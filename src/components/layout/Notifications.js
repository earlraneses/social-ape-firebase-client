...
class Notifications extends Component {
  ...
  wrapWithBadge = (notificationsIcon, count) => {
    return (
  ...
  render() {
    ...
    if (hasNotifications) {
      const count = notifications.filter((notif) => notif.read === false)
        .length;
      if (count > 0) {
        notificationsIcon = this.wrapWithBadge(notificationsIcon, count);
      }
    }

    const notificationsMarkup = hasNotifications ? (
